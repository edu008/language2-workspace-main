...
export const PraepositionProvider = ({ children }) => {
  ...
  const stats = useMemo(() => {
    ...
    const totalLearnedCount = standingSummary.praeposition.filter(s => s.correct === 2).length;
    
    return {
      totalCount: data.praeposition.length,
      trainedCount: totalLearnedCount,
      progress: data.praeposition.length > 0 
        ? Math.round((totalLearnedCount / data.praeposition.length) * 100) || 0 
        : 0,
    };
  }, [data.praeposition, standingSummary.praeposition]);
  ...
  const selectNextWord = () => {
    ...
    console.log("Selected word:", selectedWord?.Satz || "None");
  };
  ...
  const handleWordFeedback = async (isCorrect) => {
    ...
    try {
      ...
      const updatedSummary = [...standingSummary.praeposition];
      ...
      if (index !== -1) {
        ...
        const newStandingItem = {
          ...
          Satz: processedItem.Satz || "Unbekannt",
          Loesung: processedItem.Loesung || "Keine Lösung",
        };
  ...
  const resetLearningProgress = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        setStandingSummary(prev => ({
          ...prev,
          praeposition: [] // Clear the standing summary for praeposition
        }));
        
        // Force a reload of the data to ensure everything is reset properly
        await loadInitialData("praeposition");
        setIsDataLoaded(true);
      } else {
