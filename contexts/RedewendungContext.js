...
// Create the context
export const RedewendungContext = createContext();

// Custom hook for using this context
export const useRedewendungContext = () => useContext(RedewendungContext);

export const RedewendungProvider = ({ children }) => {
  ...
  const stats = useMemo(() => {
    ...
    const totalLearnedCount = standingSummary.redewendung.filter(s => s.correct === 2).length;
    
    return {
      totalCount: data.redewendung.length,
      trainedCount: totalLearnedCount,
      progress: data.redewendung.length > 0 
        ? Math.round((totalLearnedCount / data.redewendung.length) * 100) || 0 
        : 0,
    };
  }, [data.redewendung, standingSummary.redewendung]);

  // Load initial data when component mounts or route changes
  useEffect(() => {
    if (router.pathname === "/redewendung" && status === "authenticated" && !isDataLoaded) {
      loadRedewendungData();
    }
  ...
  // Load redewendung data
  const loadRedewendungData = async () => {
    const featureData = await loadInitialData("redewendung");
    if (featureData && featureData.length > 0) {
      applyStandingToWords(standingSummary.redewendung, featureData);
    }
  };
  ...
  const selectNextWord = () => {
    ...
    console.log("Selected word:", selectedWord?.Redewendung || "None");
  };
  ...
  const handleWordFeedback = async (isCorrect) => {
    ...
    try {
      ...
      const standingItem = standingSummary.redewendung.find(
      ...
      await saveToServer("redewendung", isCorrect ? "OK" : "NOK", updatedStanding);
      
      // Update the standing summary in DataContext
      const updatedSummary = [...standingSummary.redewendung];
      ...
      if (index !== -1) {
        ...
        const newStandingItem = {
          ...
          Redewendung: processedItem.Redewendung || "Unbekannt",
      ...
      console.error("[handleWordFeedback:redewendung] Fehler:", error);
    }
  };
  ...
  const resetLearningProgress = async () => {
    ...
    try {
      const result = await saveToServer("redewendung", "REV", null);
      if (result) {
        console.log("Server-Reset für redewendung erfolgreich:", result);
        
        // Update local state directly without reloading data
        setPools({
          untrained: data.redewendung,
          repeat: [],
          learned: [],
        });
        ...
        setStandingSummary(prev => ({
          ...prev,
          redewendung: [] // Clear the standing summary for redewendung
        }));
        
        // Force a reload of the data to ensure everything is reset properly
        await loadInitialData("redewendung");
        setIsDataLoaded(true);
      } else {
        console.error("Server-Reset für redewendung fehlgeschlagen");
      }
    } catch (error) {
      console.error("[resetLearningProgress:redewendung] Fehler:", error);
    }
  };

  return (
    <RedewendungContext.Provider
      value={{
        // State
        currentItem,
        pools,
