...
// Create the context
export const DeutschContext = createContext();

// Custom hook for using this context
export const useDeutschContext = () => useContext(DeutschContext);

export const DeutschProvider = ({ children }) => {
  ...
  const { applyFilters, filteredData } = useFilterContext();
  ...
  const stats = useMemo(() => {
    ...
    const totalLearnedCount = standingSummary.deutsch.filter(s => s.correct === 2).length;
    
    return {
      totalCount: data.deutsch.length,
      trainedCount: totalLearnedCount,
      progress: data.deutsch.length > 0 
        ? Math.round((totalLearnedCount / data.deutsch.length) * 100) || 0 
        : 0,
    };
  }, [data.deutsch, standingSummary.deutsch]);
  ...
  // Load deutsch data
  const loadDeutschData = async () => {
    const featureData = await loadInitialData("deutsch");
    if (featureData && featureData.length > 0) {
      applyStandingToWords(standingSummary.deutsch, featureData);
    }
  };
  ...
  // Update pools from filters
  const updatePoolsFromFilters = (filteredPools) => {
    // Update the pools
    setPools(filteredPools);
    
    // Always select a new word when filters are applied
    
    let selectedWord = null;
    let availableWords = [];

    // Check if it's time to show a repeat word
    if (counters.wordCounter >= counters.nextRepeatAt && filteredPools.repeat.length > 0) {
      // Select a word from the repeat pool
      availableWords = filteredPools.repeat.filter(word => 
        !counters.seenWords.includes(word.id.toString())
      );
      
      // If all repeat words have been seen recently, use any repeat word
      if (availableWords.length === 0) {
        availableWords = filteredPools.repeat;
      }
      ...
      availableWords = filteredPools.untrained.filter(word => 
        !counters.seenWords.includes(word.id.toString())
      );
      
      // If all untrained words have been seen recently, use any untrained word
      if (availableWords.length === 0 && filteredPools.untrained.length > 0) {
        availableWords = filteredPools.untrained;
      }
    ...
    if (!selectedWord && !filteredPools.untrained.length && !filteredPools.repeat.length) {
      setCurrentItem(null);
    } else {
      setCurrentItem(selectedWord);
      console.log("Selected word from filter:", selectedWord?.Word || "None");
    }
  };
  ...
  const selectNextWord = () => {
    ...
    console.log("Selected word:", selectedWord?.Word || "None");
  };
  ...
  const handleWordFeedback = async (isCorrect) => {
    ...
    try {
      ...
      const standingItem = standingSummary.deutsch.find(
      ...
      await saveToServer("deutsch", isCorrect ? "OK" : "NOK", updatedStanding);
      
      // Update the standing summary in DataContext
      const updatedSummary = [...standingSummary.deutsch];
      ...
      if (index !== -1) {
        ...
        const newStandingItem = {
          ...
          Word: processedItem.Word || "Unbekannt",
          Artikel: processedItem.Artikel || "",
          Transl_F: processedItem.Transl_F?.[0]?.Transl_F || "Keine Übersetzung",
        };
      ...
      console.error("[handleWordFeedback:deutsch] Fehler:", error);
    }
  };
  ...
  const resetLearningProgress = async () => {
    ...
    try {
      const result = await saveToServer("deutsch", "REV", null);
      if (result) {
        console.log("Server-Reset für deutsch erfolgreich:", result);
        
        // Update local state directly without reloading data
        setPools({
          untrained: data.deutsch,
          repeat: [],
          learned: [],
        });
        ...
        setStandingSummary(prev => ({
          ...prev,
          deutsch: [] // Clear the standing summary for deutsch
        }));
        
        // Force a reload of the data to ensure everything is reset properly
        await loadInitialData("deutsch");
        setIsDataLoaded(true);
      } else {
        console.error("Server-Reset für deutsch fehlgeschlagen");
      }
    } catch (error) {
      console.error("[resetLearningProgress:deutsch] Fehler:", error);
    }
  };

  return (
    <DeutschContext.Provider
      value={{
        // State
        currentItem,
        pools,
