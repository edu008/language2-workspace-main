...
// Einfache Komponente für die Anzeige der Präposition (analog zu WordCard)
const PraepositionCard = ({ wordData, showTranslation, onFlip }) => (
  <div className="bg-white rounded-lg p-4 shadow-lg" onClick={onFlip}>
    <div className="text-4xl text-center">
      {wordData.satz}
      {showTranslation && (
        <div>
          <strong>Lösung:</strong> {wordData.loesung}
        </div>
      )}
    </div>
...
export default function Praeposition({ praepositionCount, praeposition }) {
  ...
  const applyStandingToWords = (standing) => {
    if (!standing || standing.length === 0) {
      ...
      const untrainedWords = praeposition.filter(p => 
        !learnedIds.includes(p.id) && !repeatIds.includes(p.id) && !nokIds.includes(p.id)
      );
      const repeatPoolWords = praeposition.filter(p => 
        repeatIds.includes(p.id) || nokIds.includes(p.id)
      );
  ...
  const applyNextPraeposition = async () => {
    if (isApplyingFilters || (!Array.isArray(untrained) || !Array.isArray(repeatPool))) {
      console.error("Ungültige Pools:", { untrained, repeatPool });
      return;
    }
  ...
  const handleOK = async () => {
    ...
    try {
      ...
      setUntrained(prev => prev.filter(p => p.id !== exercise));
  ...
  const handleNOK = async () => {
    ...
    try {
      ...
      setRepeatPool(prev => prev.filter(p => p.id !== exercise));
