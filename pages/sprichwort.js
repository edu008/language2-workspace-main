...
export default function Sprichwort({ sprichwortCount, sprichwort, standingSums, summary }) {
  ...
  const handleOK = async () => {
    const response = await fetch(`/api/standing?user=${encodeURIComponent(session.user.email)}&exercise=${encodeURIComponent(sprichwort.id)}`)
      ...
      return (
        <div className="h-144 bg-white rounded-lg p-4 shadow-lg relative flex items-center">
          ...
          <div className="relative z-20 w-full max-w-full text-center">
            <p className="text-2xl mx-auto font-medium">{sprichwort.Sprichwort}</p>
            <div className="text-left mt-28">
              <p className="text-xl mx-auto font-semibold">Hauptwort</p>
              <p className="text-xl mb-4 mx-auto">{sprichwort.Wort}</p>
              <p className="text-xl mx-auto font-semibold mt-5">Beispiele</p>
              <p className="mx-auto">{sprichwort.Beispiel}</p>
            </div>
          </div>
          <div className="absolute bottom-0 right-0 mb-5 mr-5 text-right">
            <p className="text-sm">Quelle: {sprichwort.Quelle}</p>
            <p className="text-sm">Hinzugefügt am: {sprichwort.Datum}</p>
          </div>
        </div>
      );
    }
    ...
    const Content2 = () => {
      return (
        <div className="h-144 bg-white rounded-lg p-4 shadow-lg relative flex items-center">
          <div className="absolute top-0 bottom-0 left-0 right-0 bg-green-200 opacity-50 z-10"></div>
          <div className="relative z-20 w-full max-w-full text-center">
            <p className="text-2xl mx-auto font-medium">{sprichwort.Erklaerung}</p>
          </div>
          <div className="absolute bottom-0 right-0 mb-5 mr-5 text-right">
            <p className="text-sm">Quelle: {sprichwort.Quelle}</p>
            <p className="text-sm">Hinzugefügt am: {sprichwort.Datum}</p>
        ...
        <div className="max-w-5xl mx-auto py-2 px-4 sm:px-6 lg:px-8 mt-4">
          <div className="grid grid-cols-4 gap-4 mb-4">
            <div className="flex flex-col items-center justify-center bg-gray-200 rounded-lg p-4">
              <p className="text-2xl font-bold">{sprichwortCount}</p>
        ...
        {summary.length > 0 && (
          <div>
            ...
            <table className="table-auto">
              <thead>
                <tr>
                  <th className="px-4 py-2">Sprichwort</th>
                  <th className="px-4 py-2">Erklärung</th>
                </tr>
              </thead>
              <tbody>
                {summary.map((obj) => {
                  const sprichwort = obj.summary;
                  return (
                    <tr>
                      <td className="border px-4 py-2">
                        <p className="normalp">{sprichwort.Sprichwort}</p>
                      </td>
                      <td className="border px-4 py-2">
                        <p className="normalp">{sprichwort.Erklaerung}</p>
                      </td>
                    </tr>
                  );
                })}
